...
data_long <- data_long %>%
  mutate(ps_w_long = w.out1$weights)
...
# data_long %>%
#   ggplot(aes(w_smoke, w_smoke, color = smoke_dic)) +
#   geom_point()

data_long %>%
  ggplot(aes(ps_w_long, w_smoke, color = smoke_dic)) +
  geom_point()
...
cde_adjusted <- risks_boot_long(data_long, n = 3, seed = 123, iptw = TRUE)
